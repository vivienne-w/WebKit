...
void webKitMediaSrcEmitStreams(WebKitMediaSrc* source, const Vector<RefPtr<MediaSourceTrackGStreamer>>& tracks)
{
...
#ifndef GST_DISABLE_GST_DEBUG
        GST_DEBUG_OBJECT(source, "Adding stream with trackId '%lu' of type %s with caps %" GST_PTR_FORMAT, track->id(), streamTypeToString(track->type()), track->initialCaps().get());
#endif // GST_DISABLE_GST_DEBUG
        if (source->priv->streams.contains(track->id())) {
            GST_ERROR_OBJECT(source, "stream with trackId '%lu' already exists", track->id());
            ASSERT_NOT_REACHED();
            continue;
        }
    ...
    for (const RefPtr<Stream>& stream: source->priv->streams.values()) {
        ...
        GST_DEBUG_OBJECT(source, "Adding pad '%s' for stream with id '%lu'", GST_OBJECT_NAME(stream->pad.get()), stream->track->id());
...
static void webKitMediaSrcTearDownStream(WebKitMediaSrc* source, TrackID id)
{
    ...
    GST_DEBUG_OBJECT(source, "Tearing down stream '%lu'", id);
...
static void webKitMediaSrcLoop(void* userData)
{
    ...
    if (!streamingMembers->wasStreamStartSent) {
        GUniquePtr<char> streamId { g_strdup_printf("mse/%lu", stream->track->id()) };
...
static void webKitMediaSrcStreamFlush(Stream* stream, bool isSeekingFlush)
{
    ...
    GST_DEBUG_OBJECT(stream->source, "Flush requested for stream '%lu'. isSeekingFlush = %s",
...

    {
        ...
        if (!streamingMembers->hasPoppedFirstObject) {
            GST_DEBUG_OBJECT(stream->source, "Flush request for stream '%lu' occurred before hasPoppedFirstObject, just clearing the queue and readjusting the segment.", stream->track->id());
            DataMutexLocker queue { stream->track->queueDataMutex() };
    ...
    GST_DEBUG_OBJECT(stream->source, "Flush request for stream '%lu' (isSeekingFlush = %s) satisfied.",
        stream->track->id(), boolForPrinting(isSeekingFlush));
}
...
{
    ASSERT(isMainThread());
    GST_DEBUG_OBJECT(source, "Received non-seek flush request for stream '%lu'.", streamId);
